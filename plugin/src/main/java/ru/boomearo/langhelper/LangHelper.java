...
import ru.boomearo.langhelper.api.LangHelperApi;
...
import ru.boomearo.langhelper.versions.TranslationVersionWrapper;
...
public class LangHelper extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        try {
            ...
            for (TranslatedMessages messages : this.translateManager.getAllTranslate()) {
                String languageName = messages.getTranslate("language.name");
                if (languageName == null) {
                    languageName = "Unknown-name";
                }
                String languageRegion = messages.getTranslate("language.region");
                if (languageRegion == null) {
                    languageRegion = "Unknown-region";
                }
                this.getLogger().info("Language '" + messages.getLangType().name() + " [" + languageName + "-" + languageRegion + "]' successfully loaded. Translation keys: " + messages.getAllTranslate().size());
            }

            LangHelperApi.set(this.translateManager);
    ...
    private DefaultTranslateManager matchVersion(Plugin plugin, ConfigManager configManager) throws LangVersionException {
        try {
            ...
            return (DefaultTranslateManager) Class.forName("ru.boomearo.langhelper.versions." + foundClass)
                    .getDeclaredConstructor(Plugin.class, ConfigManager.class)
